...
class ArtifactControllerTest {
    ...
    void findAll() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/artifacts").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()).andExpect((ResultMatcher) jsonPath("$.data",hasSize(2)))
                .andExpect((ResultMatcher) jsonPath("$.data[0].id").value(("1250808601736515584")))
                .andExpect((ResultMatcher) jsonPath("$.data[0].name").value("Deluminator"))
                .andExpect((ResultMatcher) jsonPath("$.data[1].id").value("1250808601744904193"))
                .andExpect((ResultMatcher) jsonPath("$.data[1].name").value("Invisibility Cloak"));
